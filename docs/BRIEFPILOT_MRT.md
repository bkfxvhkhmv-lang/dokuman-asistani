...
| `pending current commit` | fix(speech): show read-aloud in normal detail flow — `DocumentSpeechSection` now renders in `DetailDetailsTab`; Vorlesen is source-independent and text-dependent in the normal detail user flow |
