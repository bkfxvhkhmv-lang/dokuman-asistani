# Approved German MVP Reply Templates

Date: 2026-06-03
Status: Source of truth
Scope: First 20 content-approved German Reply Assistant templates

Rules:

- Placeholder keys use `lowercase_snake_case`
- Conditional blocks use `{{#if field_key}}...{{/if}}`
- This document is source material only
- No registry import is implied by this file

## BUẞGELD (1–5)

### 1. `bussgeld_akten_einsicht_009`
...
```text
Dieser Antrag dient ausschließlich der Informationsbeschaffung. Er äußert sich nicht zur Schuldfrage. Beachten Sie, dass für die Akteneinsicht oder die Zusendung von Kopien gesetzliche Gebühren anfallen können.
```

### 2. `bussgeld_fristverlaengerung_010`
...
```text
Antrag auf Fristverlängerung zur Stellungnahme – Aktenzeichen: {{aktenzeichen}}
```
...
```text
Das Einreichen dieses Antrags garantiert nicht, dass die Bußgeldstelle die Frist verlängert. Senden Sie das Schreiben rechtzeitig ab und prüfen Sie kurz vor Ablauf der ursprünglichen Frist den Status, um Nachteile zu vermeiden.
```

### 3. `bussgeld_photo_anfordern_013`
...
```text
Anforderung des Beweisfotos – Aktenzeichen: {{aktenzeichen}}
```
...
Um den Vorgang prüfen und den Fahrzeugführer zum fraglichen Zeitpunkt eindeutig identifizieren zu können, bitte ich hiermit um die Zusendung des entsprechenden Beweisfotos bzw. der Messdaten.

Soweit mir bereits ein Foto vorliegt, bitte ich um Übersendung einer gut lesbaren Kopie. Falls mir bisher kein Foto übersandt wurde, bitte ich um erstmalige Übersendung des vorhandenen Beweisfotos.
...
```text
Die Anforderung eines Beweisfotos unterbricht oder stoppt laufende Fristen (z. B. die Einspruchsfrist gegen einen bereits erlassenen Bußgeldbescheid) im Regelfall nicht. Reagieren Sie daher trotzdem innerhalb der gesetzlichen Fristen.
```

### 4. `bussgeld_parking_einspruch_003`
...
```text
Einspruch gegen den Bußgeldbescheid / Verwarnung wegen Parkverstoß – Aktenzeichen: {{aktenzeichen}}
```
...
```text
Begründen Sie Ihren Einspruch im Feld "einspruch_begruendung" sachlich und wahrheitsgemäß (z. B. defekter Parkscheinautomat, gültiger Parkausweis lag aus). Das Risiko bei Ablehnung des Einspruchs ist der Übergang in ein teureres gerichtliches Verfahren.
```

### 5. `bussgeld_general_einspruch_005`

- `id`: `bussgeld_general_einspruch_005`
- `title`: `Allgemeiner Einspruch gegen den Bußgeldbescheid`
- `riskLevel`: `high`
- `requiredFields`: `["aktenzeichen", "name", "adresse"]`
- `optionalFields`: `[]`
- `tone`: `Formell & Bestimmt`
- `legalRiskNotes`: `Einspruch gegen einen allgemeinen Bußgeldbescheid (z. B. Geschwindigkeitsverstoß). Hohes Risiko, falls der Einspruch unbegründet ist, da bei einer Abgabe an das Gericht zusätzliche Gerichts- und Verfahrenskosten entstehen.`
- `subject`:
...
Der Einspruch erfolgt zunächst ohne nähere Begründung. Eine ausführliche Begründung werde ich nachreichen, sobald mir alle hierfür erforderlichen Unterlagen vorliegen und ich den Vorgang vollständig prüfen konnte.
...
```text
Die Frist für den Einspruch beträgt strikt zwei Wochen ab Zustellung des Bußgeldbescheids. Formulieren Sie spätere Begründungen präzise und wahrheitsgemäß. Bei Ablehnung des Einspruchs drohen Gerichtsverfahren und zusätzliche Gebühren.
```

## JOBCENTER (6–10)

### 6. `jobcenter_unterlagen_nachreichung_004`
...
- `legalRiskNotes`: `Keine rechtlichen Risiken. Das Schreiben dient dem Nachweis, dass der Bürger seiner Mitwirkungspflicht (Mitwirkungspflicht) nachgekommen ist.`
- `subject`:

```text
Nachreichung von Unterlagen – BG-Nr. {{bg_nummer}}
```
...
```text
Bitte listen Sie im Feld "unterlagen_liste" alle Dokumente übersichtlich auf. Vergewissern Sie sich, dass die Unterlagen vollständig sind, um weitere Verzögerungen bei der Auszahlung zu vermeiden.
```

### 7. `jobcenter_fristverlaengerung_005`
...
```text
Antrag auf Fristverlängerung zur Mitwirkung – BG-Nr. {{bg_nummer}}
```
...
```text
Das Jobcenter ist nicht gesetzlich verpflichtet, einer Fristverlängerung zuzustimmen. Senden Sie dieses Schreiben rechtzeitig vor Ablauf der ursprünglichen Frist ab, um Leistungskürzungen zu vermeiden.
```

### 8. `jobcenter_sachstandsanfrage_untaetigkeit_013`
...
```text
Sachstandsanfrage zu meinem Antrag vom {{antrag_datum}} – BG-Nr. {{bg_nummer}}
```
...
```text
Je nach Verfahrensart gelten unterschiedliche Bearbeitungs- und Rechtsbehelfsfristen. Prüfen Sie daher das Datum Ihres Antrags und vorhandene Bescheide sorgfältig.
```

### 9. `jobcenter_antrag_wba_007`
...
```text
Einreichung des Weiterbewilligungsantrags (WBA) – BG-Nr. {{bg_nummer}}
```
...
```text
Bitte prüfen Sie sorgfältig, ob sich Einkommen, Wohnkosten, Bankverbindung oder die Personen in Ihrer Bedarfsgemeinschaft geändert haben. Änderungen müssen wahrheitsgemäß angegeben werden.
```

### 10. `jobcenter_antrag_vorschuss_006`
...
```text
Eilantrag auf einen Vorschuss / eine Vorschusszahlung – BG-Nr. {{bg_nummer}}
```
...
```text
Ein Vorschussantrag setzt voraus, dass Ihr Anspruch dem Grunde nach besteht und Sie mittellos sind. Beschreiben Sie Ihre Notlage absolut wahrheitsgemäß. Geben Sie Bankdaten nur ein, wenn Sie sicher sind, dass der Empfänger die zuständige Behörde ist.
```

## FINANZAMT (11–14)

### 11. `finanzamt_fristverlaengerung_003`
...
```text
Antrag auf Fristverlängerung – Steuernummer: {{steuernummer}}
```
...
```text
Das Finanzamt ist nicht verpflichtet, Fristverlängerungen (insbesondere rückwirkend) zu gewähren. Reichen Sie diesen Antrag rechtzeitig vor Ablauf der aktuellen Frist ein.
```

### 12. `finanzamt_ratenzahlung_005`
...
```text
Antrag auf Ratenzahlung – Steuernummer: {{steuernummer}}
```
...
```text
Ein Antrag auf Ratenzahlung schützt Sie nicht automatisch vor Säumniszuschlägen oder Vollstreckungsmaßnahmen. Die Behörde fordert in der Regel zusätzliche Nachweise über Ihre Einnahmen und Ausgaben. Machen Sie im Begründungsfeld nur absolut wahrheitsgemäße Angaben.
```

### 13. `finanzamt_akteneinsicht_007`
...
```text
Antrag auf Akteneinsicht – Steuernummer: {{steuernummer}}
```
...
```text
Im Steuerrecht besteht nicht in jedem Fall ein voraussetzungsloser Anspruch auf Akteneinsicht. Das Finanzamt entscheidet regelmäßig nach pflichtgemäßem Ermessen. Dieser Antrag stoppt keine laufenden Einspruchsfristen!
```

### 14. `finanzamt_einspruch_fristwahrung_002`
...
```text
Einspruch zur Fristwahrung gegen den {{bescheid_typ}} vom {{bescheid_datum}} – Steuernummer: {{steuernummer}}
```
...
```text
Ein Einspruch hemmt die Zahlungspflicht grundsätzlich nicht. Prüfen Sie daher die im Bescheid genannte Zahlungsfrist. Wenn die Zahlung vorläufig ausgesetzt werden soll, kann zusätzlich ein gesonderter Antrag auf Aussetzung der Vollziehung erforderlich sein.
```

## MIETRECHT & NEBENKOSTEN (15–18)

### 15. `miete_mangel_anzeige_004`
...
```text
Mängelanzeige bezüglich der Mietwohnung – {{mietwohnung_lage}}
```

- `body`:

```text
Sehr geehrte(r) Frau/Herr {{vermieter_name}},
...
```text
Eine Mängelanzeige ist der erste notwendige Schritt, bevor rechtliche Ansprüche geltend gemacht werden können. Dokumentieren Sie den Mangel unbedingt zusätzlich mit Fotos und führen Sie ein Protokoll. Mindern Sie die Miete nicht eigenständig ohne rechtliche Beratung.
```

### 16. `miete_reparatur_fristsetzung_005`
...
```text
Fristsetzung zur Mängelbeseitigung – Mietwohnung: {{mietwohnung_lage}}
```
...
```text
Die gesetzte Frist muss angemessen sein (je nach Art des Mangels meistens zwischen 2 Wochen und einem Monat). Setzen Sie kein zu kurzes Datum, da die Fristsetzung sonst rechtlich unwirksam sein könnte. Nach fruchtlosem Ablauf der Frist können weitere Rechte entstehen.
```

### 17. `miete_belegeinsicht_002`
...
```text
Bitte um Belegeinsicht zur Abrechnung – Zeitraum: {{abrechnungs_zeitraum}}
```
...
```text
Der Mieter hat grundsätzlich ein Recht darauf, die Originalbelege beim Vermieter einzusehen, bevor er Nachzahlungen leistet. Eine pauschale Verweigerung der Zahlung ohne vorherige Bitte um Belegeinsicht ist jedoch riskant. Klären Sie vorab, ob für Kopien Kosten anfallen.
```

### 18. `nk_belegeinsicht_002`
...
```text
Bitte um Belegeinsicht zur Nebenkostenabrechnung {{abrechnungs_jahr}}
```
...
```text
Die Anforderung von Belegen entbindet Sie im Regelfall nicht von der Einhaltung der gesetzlichen Einwendungsfrist von 12 Monaten nach Erhalt der Abrechnung. Prüfen Sie, ob Ihr Mietvertrag eine Pauschale für Kopierkosten vorsieht.
```

## SCHUFA & INKASSO (19–20)

### 19. `schufa_selbstauskunft_001`
...
```text
Antrag auf Erteilung einer kostenlosen Datenkopie nach Art. 15 DSGVO
```
...
```text
Nach Art. 12 Abs. 3 DSGVO muss die Auskunft unverzüglich, spätestens jedoch innerhalb eines Monats erteilt werden. Zur eindeutigen Identifikation kann es in einigen Fällen notwendig sein, dem Schreiben eine Kopie des Personalausweises (Vorder- und Rückseite, sensible Daten geschwärzt) beizufügen.
```

### 20. `inkasso_zahlung_nachweis_003`
...
```text
Nachweis der Zahlung zur Forderung – Az. {{aktenzeichen}}
```
...
```text
Achten Sie darauf, im Feld "zahlungs_nachweis_details" das genaue Datum der Überweisung, den Betrag und den Verwendungszweck anzugeben. Fügen Sie dem fertigen Schreiben unbedingt eine Kopie des Überweisungsbelegs (z. B. Kontoauszug oder Einzahlungsbeleg) bei.
```
