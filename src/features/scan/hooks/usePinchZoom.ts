...
export function usePinchZoom(): PinchZoomState {
  ...
  const stepZoom = useCallback((next: number) => {
    const target = clampScale(next);
    currentScale.current = target;
    if (target <= 1) currentOffset.current = { x: 0, y: 0 };
    Animated.parallel([
      Animated.spring(scale, { toValue: target, useNativeDriver: true, damping: 16, stiffness: 220 }),
      ...(target <= 1
        ? [
            Animated.spring(translateX, { toValue: 0, useNativeDriver: true, damping: 16, stiffness: 220 }),
            Animated.spring(translateY, { toValue: 0, useNativeDriver: true, damping: 16, stiffness: 220 }),
  ...
  const zoomIn  = useCallback(() => { stepZoom(currentScale.current + 0.6); }, [stepZoom]);
  const zoomOut = useCallback(() => { stepZoom(currentScale.current - 0.6); }, [stepZoom]);
  const getScale = useCallback(() => currentScale.current, []);
