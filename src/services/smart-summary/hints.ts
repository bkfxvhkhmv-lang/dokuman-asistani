...
export function buildHandlungsempfehlungen(dok: Dokument): string[] {
  ...
  if (tage !== null && tage < 0 && canOfferPaymentAction(dok.betrag)) empfehlungen.push('🚨 Zahlung sofort veranlassen — Frist abgelaufen');
  else if (tage !== null && tage <= 3 && canOfferPaymentAction(dok.betrag)) empfehlungen.push(`💶 Zahlung in ${tage} Tag${tage !== 1 ? 'en' : ''} fällig — jetzt vorbereiten`);
